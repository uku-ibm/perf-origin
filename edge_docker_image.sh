...
delete_existing_edge_runtime_from_tenant(){
    ...
    response=$(curl -s -H "authtoken:$AUTH_TOKEN" -H "cookie:$COOKIE" -H "X-csrf-token:$CSRF" "https://$TENANT_URL/integration/rest/edge/runtimes?page=1&limit=19&searchKey=$REMOTE_EDGE_SERVER_NAME")
    ...
    if [ -z "$response" ]; then
        ...
        if [ -z "$agentID" ]; then
            ...
            response=$(curl -s -X DELETE -H "authtoken:$AUTH_TOKEN" -H "cookie:$COOKIE" -H "X-csrf-token:$CSRF" "https://$TENANT_URL/integration/rest/edge/runtimes/$agentID/deregister?force=true")
...
verify_edge_runtime_on_tenant_runtime_dashboard(){
    ...
    # request to run
    response=$(curl -s -H "authtoken:$AUTH_TOKEN" -H "cookie:$COOKIE" -H "X-csrf-token:$CSRF" "https://$TENANT_URL/integration/rest/edge/runtimes/$agentID/instances/")
    echo "RESPONSE: $response"
...
configure_edge_runtime_to_tenant(){
     ...
     response=$(curl -s -X POST -H "Content-Type: application/json" -H "authtoken:$AUTH_TOKEN" -H "cookie:$COOKIE" -H "X-csrf-token:$CSRF" -d "{\"name\":\"$REMOTE_EDGE_SERVER_NAME\",\"description\":\"Remote edge runtime for performance test\"}" "https://$TENANT_URL/integration/rest/edge/runtimes?pairInstance=true")
...
#curl -s -X POST -H "Content-Type: application/json" -H "authtoken:fl92c35f89af67a433f71657" -H "cookie:route=1711024777.792.39.1540|58fb03e5a1678771a75dea209604055b;JSESSIONID=9BB0BAB6615AFA902A3FC61A2C5EBD75;login=;lang=en;UserType=Platform" -H "X-csrf-token:dbc5be18-b7b9-4f20-aa02-ed8ca1fcd075" -d "{\"name\":\"perftest123\",\"description\":\"Remote edge runtime for performance test\"}" "https://originawsint2.int-aw-us1.webmethods-int.io/integration/rest/edge/runtimes?pairInstance=true"
    if [ -z "$response" ]; then
        ...
        if [ -z "$pairingCode" ]; then
            ...
            docker run -p 5555:5555 -d -e SAG_IS_CLOUD_REGISTER_URL=https://$TENANT_URL -e JAVA_MIN_MEM=4096M -e JAVA_MAX_MEM=4096M -e SAG_IS_EDGE_CLOUD_ALIAS=EdgeRuntime_$REMOTE_EDGE_SERVER_NAME -e SAG_IS_CLOUD_REGISTER_TOKEN=$pairingCode --name=$REMOTE_EDGE_SERVER_NAME iregistry.eur.ad.sag/aim/msr-edge-runtime:int
...
fetch_tokens_from_tenant(){
    ...
    rm -f token.json
    curl -s -u ${TENANT_USERNAME}:${TENANT_PASSWORD} https://$TENANT_URL/enterprise/v1/user/token > token.json
...
if [[ $1 == *"delete_existing_edge_runtime_from_tenant"* ]]; then
        fetch_tokens_from_tenant
        delete_existing_edge_runtime_from_tenant $CURRENT_DIRECTORY
fi
if [[ $1 == *"configure_edge_runtime_on_tenant"* ]]; then
        fetch_tokens_from_tenant
        configure_edge_runtime_on_tenant $CURRENT_DIRECTORY
fi